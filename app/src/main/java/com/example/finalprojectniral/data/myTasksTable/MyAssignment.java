...
public class MyAssignment implements Serializable { // تعريف كلاس المهمة
    ...
    public int importance; // متغير لتخزين درجة أهمية المهمة (1-5)
    public int workLoad; // متغير لتخزين مدى العمل المطلوب (1-10)
...
/* <<<<<<<<<<  6db93eed-b0e9-48d7-8bc1-3208c5154f79  >>>>>>>>>>> */
        switch (priority.toLowerCase()) { // فحص النص بعد تحويله لحروف صغيرة
            case "high":
                return 5; // عالية تأخذ القيمة 5
            case "medium":
                return 3; // متوسطة تأخذ القيمة 3
            case "low":
                return 1; // منخفضة تأخذ القيمة 1
            default:
                return 3; // القيمة الافتراضية هي 3
        }
    }
    ...
    public int getWorkLoad() { // دالة للحصول على مدى العمل
        return workLoad;
    }

    public void setWorkLoad(int workLoad) { // دالة لتعيين مدى العمل
